...
class VideoInfo:
    ...
    def from_video_path(cls, video_path: str) -> VideoInfo:
        ...
        video.release()
        return VideoInfo(width, height, fps, duration, total_frames)
...
def get_video_frames_batch_generator(
    source_path: str, batch_size: int, stride: int = 1, start: int = 0, end: Optional[int] = None
) -> Generator[list[np.ndarray], None, None]:
    """
    Get a generator that yields lists of frames from the video.
    ...
    Returns:
        (Generator[list[np.ndarray]]): A generator that yields lists of frames
            from the video.
    """
    video, start, end = _validate_and_setup_video(source_path, start, end)
    frame_position = start
